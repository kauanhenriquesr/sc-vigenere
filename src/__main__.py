...
def main():
    ...
    try:
        ...
        freq_plain = frequency(plain_text)
        
        print("Top 5 frequências no texto cifrado:")
        sorted_freq = sorted(freq_encrypted.keys(), reverse=True)
        for i, freq_val in enumerate(sorted_freq[:5]):
            char_info = freq_encrypted[freq_val][0]
            print(f"  {i+1}. {freq_val:.2f}%: '{char_info[1]}' ({char_info[2]} ocorrências)")
